...
router.post(
  ...
  wrapAsync(async (req, res) => {
    const link = new URL(req.headers.referer);
    await DepartmentsService.bulkImport(req, res, true, link.host);
    const payload = true;
    res.status(200).send(payload);
  }),
);
...
router.delete(
  '/:id',
  wrapAsync(async (req, res) => {
    await DepartmentsService.remove(req.params.id, req.currentUser);
    const payload = true;
    res.status(200).send(payload);
  }),
);
...
router.post(
  '/deleteByIds',
  wrapAsync(async (req, res) => {
    await DepartmentsService.deleteByIds(req.body.data, req.currentUser);
    const payload = true;
    res.status(200).send(payload);
  }),
);
 ...
 *      responses:
 *        200:
 *          description: Departments list successfully received
 *          content:
 *            application/json:
 *              schema:
 *                type: array
 ...
 *      responses:
 *        200:
 *          description: Departments count successfully received
 *          content:
 *            application/json:
 *              schema:
 *                type: array
...
/**
 *  @swagger
 *  /api/departments/autocomplete:
 *    get:
 *      security:
 *        - bearerAuth: []
 *      tags: [Departments]
 *      summary: Find all departments that match search criteria
 *      description: Find all departments that match search criteria
 *      responses:
 *        200:
 *          description: Departments list successfully received
 *          content:
