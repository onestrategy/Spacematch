...
import {
  ...
  setRefetch,
  deleteItemsByIds,
} from '../../stores/follow_ups/follow_upsSlice';
import { useAppDispatch, useAppSelector } from '../../stores/hooks';
import { useRouter } from 'next/router';
import { Field, Form, Formik } from 'formik';
import { DataGrid, GridColDef } from '@mui/x-data-grid';
import { loadColumns } from './configureFollow_upsCols';
...
const TableSampleFollow_ups = ({
  ...
  const {
    ...
    loading,
    count,
    notify: follow_upsNotify,
    refetch,
  } = useAppSelector((state) => state.follow_ups);
  ...
  useEffect(() => {
    if (follow_upsNotify.showNotification) {
      notify(
        follow_upsNotify.typeNotification,
        follow_upsNotify.textNotification,
      );
    }
  }, [follow_upsNotify.showNotification]);
  ...
  const handleEditAction = (id: string) => {
    router.push(`/follow_ups/${id}`);
  };

  const handleViewAction = (id: string) => {
    router.push(`/follow_ups/follow_ups-view/?id=${id}`);
  };
...
export default TableSampleFollow_ups;
